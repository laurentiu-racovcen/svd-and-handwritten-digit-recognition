...
function [train, miu, Y, Vk] = magic_with_pca (train_mat, pcs)
  ...
  % se calculeaza suma fiecarei coloane
  % si aceasta se imparte la numarul de linii
  for j=1:size(train_mat,2)
    miu(j) = sum(train_mat(:,j)) / size(train_mat,1);
  endfor
